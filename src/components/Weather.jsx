...
const Weather = ({ origin, destination }) => {
  ...
  const getWeatherDescription = (weather, location) => {
    const temp = Math.round(weather.main.temp);
    const condition = weather.weather[0].description;
    const windSpeed = Math.round(weather.wind.speed * 3.6);
    const humidity = weather.main.humidity;

    let description = `Current conditions in ${location}: ${condition} with temperatures at ${temp}°C. `;
    
    if (windSpeed > 20) {
      description += `Strong winds at ${windSpeed} km/h may affect loading operations. `;
    } else if (windSpeed > 10) {
      description += `Moderate winds at ${windSpeed} km/h, good for sailing conditions. `;
    } else {
      description += `Light winds at ${windSpeed} km/h, calm sea conditions. `;
    }

    if (humidity > 80) {
      description += "High humidity may require extra care for sensitive cargo.";
    } else if (humidity < 30) {
      description += "Low humidity, excellent conditions for most cargo types.";
    } else {
      description += "Moderate humidity levels, ideal shipping conditions.";
    }
  ...
  const WeatherGraph = () => {
    ...
    const originData = originForecast.list.slice(0, 8).map(item => ({
    ...
    const destinationData = destinationForecast.list.slice(0, 8).map(item => ({
      time: new Date(item.dt * 1000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      temp: Math.round(item.main.temp),
      humidity: item.main.humidity,
      pressure: item.main.pressure,
    ...
    const combinedData = originData.map((item, index) => ({
      time: item.time,
      originTemp: item.temp,
      destinationTemp: destinationData[index]?.temp || 0,
      originHumidity: item.humidity,
      destinationHumidity: destinationData[index]?.humidity || 0,
      originWind: item.windSpeed,
      destinationWind: destinationData[index]?.windSpeed || 0,
      originPressure: item.pressure,
      destinationPressure: destinationData[index]?.pressure || 0
    }));
    ...
    return (
      <motion.div
        ...
        className="mt-6 bg-gradient-to-r from-slate-50 to-blue-50 p-6 rounded-lg border-2 border-slate-200"
      >
        <h3 className="text-2xl font-black text-primary-600 mb-6 flex items-center">
